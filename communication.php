        ...
        if (isset($_COOKIE['identity'])) {
            if(isset($_POST['add_ok'])) {
                if (isset($_POST['title']) and isset($_POST['content'])) {
                    $sql = 'INSERT INTO  communication (title, content) VALUES ("'. $_POST['title'] . '", "' . $_POST['content'] . '")';
                ...
                $sql = 'DELETE FROM communication WHERE id='.$_POST['delete_article'];
                $conn->query($sql);
                header('Location: communication.php');
            } else if (isset($_POST['modify_ok'])) {
                $sql = 'SELECT * FROM communication WHERE id ='.$_POST['modify_article'];
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                die('<form action="communication.php" method="POST">
                ...
                $sql = 'UPDATE communication SET title="' . $_POST['title'] . '", content="' . $_POST['content'] . '" WHERE id=' . $_POST['passing_index'];
                $conn->query($sql);
                header('Location: communication.php');  
            }
            if (isset($_POST['add'])) {
                ...
                echo '<form action="communication.php" method="POST">
                刪除文章:';
                ...
                echo '<form action="communication.php" method="POST">
            <input type="submit" name="add" value="新增文章">
            <input type="submit" name="modify" value="修改文章">
            <input type="submit" name="delete" value="刪除文章">
            </form>';
            } else if (!strcmp($_COOKIE['identity'], 'student')) {
                echo '<form action="communication.php" method="POST"><input type="submit" name="add" value="新增文章"></form>';
            }
        }
        ?>
        <div class="news">
            <h2 style="color: red">※不分群組</h2>
            <?php 
                $sql = 'SELECT * FROM communication';
                $result = $conn->query($sql);
                if (!$result) {
