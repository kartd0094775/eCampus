        ...
        if (isset($_COOKIE['identity'])) {
            if(isset($_POST['add_ok'])) {
                if (isset($_POST['title']) and isset($_POST['content'])) {
                    $sql = 'INSERT INTO  progress (title, content) VALUES ("'. $_POST['title'] . '", "' . $_POST['content'] . '")';
                ...
                $sql = 'DELETE FROM progress WHERE id='.$_POST['delete_article'];
                $conn->query($sql);
                header('Location: progress.php');
            } else if (isset($_POST['modify_ok'])) {
                $sql = 'SELECT * FROM progress WHERE id ='.$_POST['modify_article'];
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                die('<form action="progress.php" method="POST">
                ...
                $sql = 'UPDATE progress SET title="' . $_POST['title'] . '", content="' . $_POST['content'] . '" WHERE id=' . $_POST['passing_index'];
                $conn->query($sql);
                header('Location: progress.php');  
            }
            if (isset($_POST['add'])) {
            ...
            <input type="submit" name="delete" value="刪除">
                ...
                echo '<form action="progress.php" method="POST"><input type="submit" name="add" value="新增"></form>';
            }
        }
        ?>
        <div id="news">
