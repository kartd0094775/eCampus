        ...
        if (isset($_COOKIE['identity'])) {
            if(isset($_POST['add_ok'])) {
                if (isset($_POST['title']) and isset($_POST['content'])) {
                    $sql = 'INSERT INTO  assignment_display (title, content) VALUES ("'. $_POST['title'] . '", "' . $_POST['content'] . '")';
                ...
                $sql = 'DELETE FROM assignment_display WHERE id='.$_POST['delete_article'];
                $conn->query($sql);
                header('Location: assignment-display.php');
            } else if (isset($_POST['modify_ok'])) {
                $sql = 'SELECT * FROM assignment_display WHERE id ='.$_POST['modify_article'];
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                die('<form action="assignment-display.php" method="POST">
                ...
                $sql = 'UPDATE assignment_display SET title="' . $_POST['title'] . '", content="' . $_POST['content'] . '" WHERE id=' . $_POST['passing_index'];
                $conn->query($sql);
                header('Location: assignment-display.php');  
            }
            if (isset($_POST['add'])) {
                ...
                echo '<form action="assignment-display.php" method="POST">
                刪除文章:';
                echo '<select name="delete_article" id="delete_article">';
                $sql = 'SELECT * FROM assignment_display';
                $result = $conn->query($sql);
            ...
            <input type="submit" name="delete" value="刪除">
                ...
                echo '<form action="assignment-display.php" method="POST"><input type="submit" name="add" value="新增"></form>';
            }
        }
        ?>
        <div id="news">
