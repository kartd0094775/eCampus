...
<html lang="en">
    ...
    <body>
        ...
        <div id="important">
            <h2>重要公告</h2>
            <div>【預設公告】請遵守並協助推廣著作權、智慧財產權相關規範     2016/01/15</div>
            <p>請遵守著作權、智慧財產權相關規範，勿非法使用、影印教科書。本校設有「校園智慧財產權宣導」專頁，連結網址<a href="http://ipo.nchu.edu.tw/">http://ipo.nchu.edu.tw/</a>，亦有特約法律顧問(詳細資訊請至本校秘書室網頁查詢)，連結網址 按這裡另外，教育部磨課師辦公室設有「智財權諮詢平臺」，連結網址<a href="http://ipr.taiwanmooc.org/">http://ipr.taiwanmooc.org/</a>還有，免費授權的電子素材及電子書，連結網址<a href="http://oers.taiwanmooc.org/">http://oers.taiwanmooc.org/</a>感謝您的配合</p>
        </div>
        <?php
        ...
        if (isset($_COOKIE['identity'])) {
            if(isset($_POST['add_ok'])) {
                ...
                $sql = 'DELETE FROM news WHERE id='.$_POST['delete_article'];
                $conn->query($sql);
                header('Location: news.php');
            } else if (isset($_POST['modify_ok'])) {
                $sql = 'SELECT * FROM news WHERE id ='.$_POST['modify_article'];
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                die('<form action="news.php" method="POST">
                ...
                $sql = 'UPDATE news SET title="' . $_POST['title'] . '", content="' . $_POST['content'] . '" WHERE id=' . $_POST['passing_index'];
                $conn->query($sql);
                header('Location: news.php');  
            }
            if (isset($_POST['add'])) {
            ...
            <input type="submit" name="delete" value="刪除">
                ...
                echo '<form action="news.php" method="POST"><input type="submit" name="add" value="新增"></form>';
            }
        }
        ?>
        <div id="news">
