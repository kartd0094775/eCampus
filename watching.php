        ...
        if (isset($_COOKIE['identity'])) {
            if(isset($_POST['add_ok'])) {
                if (isset($_POST['title']) and isset($_POST['content'])) {
                    $sql = 'INSERT INTO  watching (title, content) VALUES ("'. $_POST['title'] . '", "' . $_POST['content'] . '")';
                ...
                $sql = 'DELETE FROM watching WHERE id='.$_POST['delete_article'];
                $conn->query($sql);
                header('Location: watching.php');
            } else if (isset($_POST['modify_ok'])) {
                $sql = 'SELECT * FROM watching WHERE id ='.$_POST['modify_article'];
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                die('<form action="watching.php" method="POST">
                ...
                $sql = 'UPDATE watching SET title="' . $_POST['title'] . '", content="' . $_POST['content'] . '" WHERE id=' . $_POST['passing_index'];
                $conn->query($sql);
                header('Location: watching.php');  
            }
            if (isset($_POST['add'])) {
                ...
                echo '<form action="watching.php" method="POST">
                刪除文章:';
            ...
            <input type="submit" name="delete" value="刪除">
                ...
                echo '<form action="watching.php" method="POST"><input type="submit" name="add" value="新增"></form>';
            }
        }
        ?>
        <div id="news">
